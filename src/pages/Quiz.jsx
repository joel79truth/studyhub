// Quiz.jsx – Instant loading, fixed nav, caching, and smooth transitions
// Flow: Home → Setup → Quiz → Submit → AI Grading → Results

import React, {
  useState, useEffect, useCallback, useReducer, useRef, useMemo, useTransition, memo,
} from 'react';
...
const quizCache = new Map(); // key: "subjectId_mode_count", value: { data, timestamp }
...
const chemToLatex = (text) => {
  ...
  return text.replace(/([A-Z][a-z]?)(\d+)/g, (match, element, num) => `${element}$_{${num}}$`);
};

const SafeLatex = memo(({ text }) => {
  if (!text) return null;
  try {
...
const HomeSkeleton = () => (
  <div style={{ maxWidth: 480, margin: '0 auto', padding: '20px 16px 24px' }}>
    <div style={{ marginBottom: 20 }}>
      ...
      <div className="animate-pulse" style={{ height: 28, width: 160, background: '#e2e8f0', borderRadius: 4, marginTop: 6 }} />
    </div>
    <div className="animate-pulse" style={{ background: C.card, borderRadius: 16, padding: '18px 20px', marginBottom: 14 }}>
      ...
      <div style={{ marginTop: 8, height: 12, width: 140, background: '#e2e8f0', borderRadius: 4 }} />
    </div>
    <div className="animate-pulse" style={{ background: C.card, borderRadius: 16, padding: '16px 20px', marginBottom: 14 }}>
    ...
    {[1, 2, 3].map(i => (
      <div key={i} className="animate-pulse" style={{ background: C.card, borderRadius: 14, padding: '14px 18px', marginBottom: 10, border: `1px solid ${C.border}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ width: 28, height: 28, background: '#e2e8f0', borderRadius: 8 }} />
          <div style={{ flex: 1 }}>
            <div style={{ height: 16, width: '60%', background: '#e2e8f0', borderRadius: 4 }} />
            <div style={{ height: 12, width: '40%', background: '#e2e8f0', borderRadius: 4, marginTop: 4 }} />
          </div>
        </div>
      </div>
    ))}
...
const SetupSkeleton = () => (
  <div style={{ maxWidth: 480, margin: '0 auto', padding: '20px 16px' }}>
    <div className="animate-pulse" style={{ height: 36, width: 100, background: '#e2e8f0', borderRadius: 10, marginBottom: 20 }} />
    <div className="animate-pulse" style={{ height: 28, width: '60%', background: '#e2e8f0', borderRadius: 4, marginBottom: 4 }} />
    <div className="animate-pulse" style={{ height: 14, width: '40%', background: '#e2e8f0', borderRadius: 4, marginBottom: 24 }} />
    {[1, 2, 3].map(i => (
      <div key={i} className="animate-pulse" style={{ background: C.card, borderRadius: 14, padding: '14px 16px', marginBottom: 10, border: `1px solid ${C.border}` }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ height: 12, width: '70%', background: '#e2e8f0', borderRadius: 4, marginTop: 4 }} />
          </div>
        </div>
      </div>
    ))}
    <div className="animate-pulse" style={{ display: 'flex', gap: 10, marginBottom: 28 }}>
      {[5, 10, 15].map(n => (
        <div key={n} style={{ flex: 1, height: 44, background: '#e2e8f0', borderRadius: 12 }} />
      ))}
    </div>
    <div className="animate-pulse" style={{ height: 56, background: '#e2e8f0', borderRadius: 14 }} />
  </div>
);
...
const Quiz = () => {
  ...
  const fetchDBStats = useCallback(async (uid) => {
    try {
      ...
      if (!weakError && weakData) {
        setWeakTopics(weakData.map(w => ({ topic: w.topic, mastery: w.mastery, course: w.course || 'General' })));
      }
  ...
  useEffect(() => {
    if (phase === 'active' && questions.length > 0) {
      sessionStorage.setItem('studyhub_quiz_state', JSON.stringify({ questions, currentIndex, answers, subjectId: currentSubjectId }));
    } else {
  ...
  const getAdaptiveQuiz = useCallback(async (courseId, mode, count) => {
    ...
    try {
      ...
      quizCache.set(cacheKey, { data: transformed, timestamp: now });
      return transformed;
    } catch (err) {
  ...
  const startQuiz = useCallback(async (subjectId, mode, count) => {
    ...
    setScreen('quiz');
    setIsSubmitting(false);
  }, [getAdaptiveQuiz, addToast]);

  // ── Answer handlers ───────────────────────────────────────────────────────
  const setMcq = useCallback((index, optionIndex) => {
    dispatch({ type: 'ANSWER', index, value: optionIndex });
  }, []);
  ...
  // ── Grade structured ──────────────────────────────────────────────────────
  const gradeStructured = useCallback(async (questionText, correctAnswer, userAnswer) => {
    if (!userAnswer?.trim()) {
  ...
  const submitQuiz = useCallback(async () => {
    ...
    await Promise.all(
      details.map(async (d) => {
        ...
        try {
          ...
          const token = session?.access_token;
          if (token) {
            await fetch(`${base}/api/exam/submit-answer`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
    ...
    setScreen('results');
  }, [questions, answers, gradeStructured]);
        ...
        {lastSession && (
          <div style={{ background: C.primaryMuted, borderRadius: 14, padding: '14px 18px', marginBottom: 14, border: `1px solid ${C.primaryLight}` }}>
            ...
            <div style={{ fontSize: 14, color: C.text, fontWeight: 500 }}>
              {subjects[lastSession.course_id]?.title || 'Quiz'} —{' '}
  ...
  }, [user, stats, weakTopics, todayAnswered, lastSession, subjects, subjectList, goToSetup]);
  ...
  }, [subjects, currentSubjectId, setupMode, setupCount, isSubmitting, startQuiz, goToHome]);
  ...
  }, [questions, phase, currentIndex, answers, textAnswers, imageData, setMcq, setStructured, handleImageUpload, submitQuiz, showConfirmSubmit]);
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', background: C.bg, paddingBottom: 80, display: 'flex', flexDirection: 'column' }}>
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {screen === 'home' ? <HomeSkeleton /> : <SetupSkeleton />}
        </div>
        <BottomNav />
      </div>
    );
  ...
  if (error) return (
    <div style={{ padding: '48px 20px', textAlign: 'center', maxWidth: 380, margin: '0 auto', minHeight: '100vh', background: C.bg }}>
        ...
        {showResumeModal && (
          <div style={{ position: 'fixed', inset: 0, background: C.overlay, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}>
            <div style={{ background: C.card, borderRadius: 20, padding: '28px 24px', maxWidth: 340, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => { sessionStorage.removeItem('studyhub_quiz_state'); setShowResumeModal(false); setPendingResume(null); }}
