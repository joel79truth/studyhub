...
export default function Login() {
    ...
    const handleAuthSuccess = async (user, extra = {}) => {
        setLoading(true);
        try {
            // Upsert into profiles table (creates or updates)
            ...
            // Fetch the complete profile (now safe with maybeSingle)
            const { data, error: fetchError } = await supabase
                .from('profiles')
                .select('*')
                .eq('id', user.id)
                .maybeSingle();   // ← FIX: no more 406
    ...
    const handleGoogleSignIn = async () => {
        ...
        try {
            ...
            if (error) throw error;
            // After redirect, the user will come back; we handle in useEffect below.
        } catch (err) {
    ...
    const handleProfileSubmit = async (e) => {
        ...
        try {
            ...
            const { error } = await supabase
                ...
                }, { onConflict: 'id' });   // Use onConflict to avoid duplicate key errors

            if (error) throw error;

            // Fetch updated profile for localStorage
            const { data, error: fetchError } = await supabase
                .from('profiles')
                .select('*')
                .eq('id', user.id)
